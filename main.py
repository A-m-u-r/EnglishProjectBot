...
async def сmd3_go(message: types.Message, state: FSMContext):
    answer = message.text.lower()  # Приводим ответ к нижнему регистру
    ...
    if answer=='⬅️exit':
        ...
        # Проверяем, есть ли предыдущее слово от бота
        if wordfrombot:
            # Проверяем, начинается ли слово пользователя с последней буквы слова бота
            if answer[0] != wordfrombot[-1]:
                await message.answer(f"Your word should start with the letter '{wordfrombot[-1]}'!", reply_markup=translateTranscriptionExample)
                return
